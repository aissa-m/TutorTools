const URL="../../backEnd/php/";document.addEventListener("DOMContentLoaded",function(){fetch(URL+"perfil.php").then(res=>res.json()).then(data=>{const container=document.getElementById("datos");if(data[0].asignatura==null||data[0].asignatura==""){data[0].asignatura="Sin Especificar"}const row=`
      ...
      `;container.insertAdjacentHTML("beforeend",row)}).catch(error=>{console.error("Error del servidor."+error)})});